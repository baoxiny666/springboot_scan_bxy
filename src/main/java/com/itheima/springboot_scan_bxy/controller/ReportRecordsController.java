...
import com.itheima.springboot_scan_bxy.utils.CsvUtil;
...
//    }
    ...
    //导出excell
    @RequestMapping("/export")
    public ResponseEntity<byte[]> export(String aesData, HttpServletResponse response) throws IOException {
        //定义展示excel的标题
        List<String> excelHeader = new ArrayList<>();
        //定义excel标题对应的英文字段
        List<String> excelHeaderEn = new ArrayList<>();
        ...
        // 设置要导出的文件的名字
        String fileName = "隐患上报记录"  + sdf.format(new Date());
        ...
        try {
            //获取要导出的数据
            ByteArrayOutputStream os = CsvUtil.doExport(excelHeader,excelHeaderEn,reportRecords_ExcelList);
